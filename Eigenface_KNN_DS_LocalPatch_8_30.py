...
KNNPara ={}
KNNPara['data_num'] = len(X_data_list)
KNNPara['train_num'] = len(X_data_list) - 1
KNNPara['test_num'] = 1
KNNPara['PcNo'] = len(X_data_list) - 1
KNNPara['K'] = 10
...
for testN in xrange(data_num):
    ...
    Tbegin = time.time()    
    y_test_pred, y_test = EigKNNPatch.KNNPrediction_LocalPatch(InputDataDir, KNNPara, OriginalImPara, PatchSize, pad_para, RPCA_Para)
    Tend = time.time()
