...
def ImageID(InputList):
    ...
    return ImageID
#%%
LocalPatach = '_LocalPatch'
#LocalPatach = ''
regIfLinear = 'Linear'
#regIfLinear = 'nonlinear'
organList = ["170_pancreas","187_gallbladder","29193_first_lumbar_vertebra","30325_left_adrenal_gland","30324_right_adrenal_gland"]
organNumList = [1,2,3,4,5]
root = '/media/louis/Volume/ProgramWorkResult/VisercialMAS_ANT'
rootDir = root + "/UpsampleResult/"

output_root_dir = '/media/louis/Volume/ProgramWorkResult/VisercialMAS_ANT'+'/MultiOrgan' + LocalPatach + '/'
...
for item in organList:
    currentFile = rootDir + item + '_' + regIfLinear + LocalPatach + '_UpsampleResult'
...
for item in intersection:
    ...
    for i in xrange(len(organList)):    
        ...
        for f in filelist:
            if item in f:
                labelin = f        
                labelout = root + '/MultiOrgan' + LocalPatach + '/' + organ + "_" + item        
